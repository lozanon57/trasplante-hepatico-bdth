/**
 * Web implementation of BDTH OCR — calls the Vercel proxy at bdth-proxy-ocr.vercel.app
 * which holds the Anthropic API key server-side (no CORS issues, no key exposure in browser).
 *
 * Metro bundler loads THIS file (not claude-vision.ts) for all web builds.
 */
...
/**
 * Extract BDTH form fields from a base64-encoded image.
 * On web, calls the Vercel proxy which holds the Anthropic API key server-side.
 */
export async function extractFromImage(
  base64Image: string,
  mediaType: 'image/jpeg' | 'image/png' | 'application/pdf' | string,
  seccion: Seccion,
  options: OCROptions = {}
): Promise<Record<string, unknown>> {
  const { offlineMode = false, ocrServerUrl } = options;

  // Offline mode: route to local Python server if configured
  if (offlineMode && ocrServerUrl) {
    const body = JSON.stringify({
      image_base64: base64Image,
      media_type: mediaType,
      seccion: seccion === 'receptor_implante' ? 'implante' : seccion,
      redact_pii: true,
    });
    ...
    if (!res.ok) throw new Error(`OCR server error ${res.status}: ${await res.text()}`);
    ...
    messages: [
      {
        ...
        content: [
          {
            type: 'image',
            source: { type: 'base64', media_type: safeMimeType, data: base64Image },
          },
  ...
  if (!raw) throw new Error('El modelo no devolvió contenido de texto');
  ...
  try {
    ...
    throw new Error(`Error al parsear respuesta JSON del OCR: ${clean.slice(0, 200)}`);
  }
}

// ── Date helpers (identical to claude-vision.ts) ──────────────────────────────
